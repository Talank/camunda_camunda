...
final class DocumentReferenceTest {
  ...
  void shouldRoundTripAllMetadataFieldsViaMsgPack() {
    ...
    original.setDocumentId("doc-001").setStoreId("gcs-store").setContentHash("sha256-abc");
    original
        .getMetadata()
        ...
        .setCustomProperties(
            BufferUtil.wrapArray(MsgPackConverter.convertToMsgPack(Map.of("region", "eu-west"))));
  ...
  void shouldRoundTripCustomPropertiesViaMsgPack() {
    ...
    original
        .getMetadata()
        .setCustomProperties(BufferUtil.wrapArray(MsgPackConverter.convertToMsgPack(customProps)));
