...
test.describe('document variable visualization', () => {
  ...
  test('JSON preview', async ({page, processInstancePage}) => {
    ...
    await processInstancePage.richTextEditor.waitForEditorToLoad();
    await processInstancePage.richTextEditor.hideCaret();
  ...
  test('PDF preview', async ({page, processInstancePage}) => {
    ...
    await expect(
      page.getByRole('dialog', {name: 'Preview: test_document.pdf'}),
    ).toBeVisible();
